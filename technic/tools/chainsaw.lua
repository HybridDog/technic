...
assert(technic.chainsaw == nil)

technic.chainsaw = {}
technic.chainsaw.max_charge      = 30000 -- Maximum charge of the saw
-- Gives 2500 nodes on a single charge (about 50 complete normal trees)
technic.chainsaw.charge_per_node = 12
-- Cut down tree leaves.  Leaf decay may cause slowness on large trees
-- if this is disabled.
technic.chainsaw.cut_leaves = true
...
if technic.chainsaw.cut_leaves then
...
technic.register_power_tool("technic:chainsaw", technic.chainsaw.max_charge)
...
local function recursive_dig(pos, remaining_charge)
	if remaining_charge < technic.chainsaw.charge_per_node then
	...
	remaining_charge = remaining_charge - technic.chainsaw.charge_per_node
...
local function chainsaw_dig_recursive(pos, current_charge)
...
if minetest.global_exists("treecapitator") then
	function chainsaw_dig(pos, current_charge)
--TODO
		minetest.sound_play("chainsaw", {pos = pos, gain = 1.0,
				max_hear_distance = 10})
		return chainsaw_dig_recursive(pos, current_charge)
	end
else
	function chainsaw_dig(pos, current_charge)
		minetest.sound_play("chainsaw", {pos = pos, gain = 1.0,
...
minetest.register_tool("technic:chainsaw", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		if not meta or not meta.charge or
				meta.charge < technic.chainsaw.charge_per_node then
			return
		end
		...
		if not technic.creative_mode then
			technic.set_RE_wear(itemstack, meta.charge, technic.chainsaw.max_charge)
